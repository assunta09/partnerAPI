...
# end
  ...
  def create_grants(doc, file_attributes)
    domestic_orgs = doc.search('GrantsToDomesticOrgsGrp')
    domestic_indiv = doc.search('GrantsToDomesticIndividualsGrp')
    foreign_grants = doc.search('ForeignGrantsGrp')

    domestic_orgs_hash = {}
    domestic_orgs.children.each do |node|
      domestic_orgs_hash["#{node.name}"] = node.text
    end

    domestic_indiv_hash = {}
    domestic_indiv.children.each do |node|
      domestic_indiv_hash["#{node.name}"] = node.text
    end

    foreign_grants_hash = {}
    foreign_grants.children.each do |node|
      foreign_grants_hash["#{node.name}"] = node.text
    end


  Grant.create(
    domestic_organisations: domestic_orgs_hash["TotalAmt"],
    domestic_individuals: domestic_indiv_hash["TotalAmt"],
    foreign_entities: foreign_grants_hash["TotalAmt"],
